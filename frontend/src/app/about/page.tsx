...
import { CheckCircle2, Award, Heart, Users, Clock, ArrowRight, Shield, Star, Zap, Globe, Activity } from 'lucide-react'
...
const ICON_MAP: Record<string, React.ElementType> = {
  Heart, Award, Users, Clock, Shield, Star, Zap, Globe, Activity,
  CheckCircle2,
}

const DEFAULT_IMAGE = 'https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=800&q=80'
...
const DEFAULTS = {
  ...
  stat_patients:    '5K+',
  stat_doctors:     '10+',
  accreditations:   ['RACGP Accredited Practice', 'Medicare Provider', 'Mental Health Care Plans', 'Chronic Disease Management', 'Travel Medicine', 'Allied Health Referrals'],
  values: [
  ...
  clinic_image_url: null as string | null,
}

async function getAboutSettings() {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'
    const res = await fetch(`${apiUrl}/about`, { cache: 'no-store' })
...
export default async function AboutPage() {
  const raw = await getAboutSettings()
  const d = {
    ...DEFAULTS,
    ...raw,
    accreditations: raw?.accreditations ?? DEFAULTS.accreditations,
    values:         raw?.values         ?? DEFAULTS.values,
  }
            ...
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              <AnimatedSection direction="left">
                <div className="relative rounded-3xl overflow-hidden aspect-[4/3] shadow-hover">
                  <Image
                    src={d.clinic_image_url ? getImageUrl(d.clinic_image_url) : DEFAULT_IMAGE}
                    alt="Our clinic"
                    fill className="object-cover"
                    unoptimized={!!d.clinic_image_url}
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-primary-900/30 to-transparent" />
                  <div className="absolute bottom-6 left-6 right-6 glass-card px-5 py-4">
                    <div className="flex items-center gap-3 sm:gap-4 text-white">
                      <div className="text-center"><div className="text-lg sm:text-2xl font-bold">{d.stat_years}</div><div className="text-xs text-white/70">Years</div></div>
                      <div className="w-px h-8 sm:h-10 bg-white/20" />
                      <div className="text-center"><div className="text-lg sm:text-2xl font-bold">{d.stat_patients}</div><div className="text-xs text-white/70">Patients</div></div>
                      <div className="w-px h-8 sm:h-10 bg-white/20" />
                      <div className="text-center"><div className="text-lg sm:text-2xl font-bold">{d.stat_doctors}</div><div className="text-xs text-white/70">Doctors</div></div>
                    </div>
                  </div>
                </div>
              </AnimatedSection>
              ...
              <AnimatedSection direction="right">
                ...
                <p className="text-gray-500 leading-relaxed mb-4">{d.story_paragraph1}</p>
                <p className="text-gray-500 leading-relaxed mb-6">{d.story_paragraph2}</p>
                {d.accreditations.length > 0 && (
                  <div className="flex flex-col gap-2 mb-8">
                    {d.accreditations.map((item: string) => (
          ...
          <section className="py-20 bg-medical-soft">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {d.values.map((v: { icon: string; title: string; desc: string }, i: number) => {
