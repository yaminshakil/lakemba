  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <AnimatedSection className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold text-primary-900">Add New Doctor</h2>
          <p className="text-gray-400 text-sm">Fill in the doctor&apos;s details below</p>
        </div>
      </AnimatedSection>
          ...
          <div className="card p-6 mb-5">
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-gray-400 text-xs mt-2">PNG, JPG up to 4MB. Square photos work best.</p>
              </div>
            </div>
          </div>
        </AnimatedSection>
          ...
          <div className="card p-6 mb-5">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <input name="name" className="input" required placeholder="Dr. Jane Smith" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
              </div>
              <div>
                <label className="label">Qualifications *</label>
                <input name="qualifications" className="input" required placeholder="MBBS, FRACGP" value={form.qualifications} onChange={e => setForm(f => ({ ...f, qualifications: e.target.value }))} />
              </div>
              <div>
                <label className="label">Specialty *</label>
                <input name="specialty" className="input" required placeholder="General Practice" value={form.specialty} onChange={e => setForm(f => ({ ...f, specialty: e.target.value }))} />
              </div>
              <div>
              ...
              <div className="sm:col-span-2">
                ...
                <textarea name="biography" className="input h-32 resize-none" placeholder="Doctor biography..." value={form.biography} onChange={e => setForm(f => ({ ...f, biography: e.target.value }))} />
              </div>
              <div className="sm:col-span-2 flex items-center gap-2">
                ...
                <label htmlFor="is_featured" className="text-sm text-gray-700 font-medium">Feature this doctor on the homepage</label>
              </div>
            </div>
          </div>
        </AnimatedSection>
        ...
        <AnimatedSection delay={0.1}>
          <div className="card p-6 mb-5">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={addLanguage} className="btn-teal py-2 px-4">
              ...
              {DAYS.map(day => (
                <button key={day} type="button" onClick={() => toggleDay(day)}
                  className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                    form.available_days.includes(day) ? 'bg-teal-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}>
                  {day}
                </button>
              ))}
