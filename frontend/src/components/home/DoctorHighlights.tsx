...
const AVATAR_COLORS = ['from-blue-400 to-blue-600', 'from-teal-400 to-teal-600', 'from-purple-400 to-purple-600', 'from-rose-400 to-rose-600']
          ...
          {loading
            ...
            : doctors.map((doc, i) => (
                <AnimatedSection key={doc.id} delay={i * 0.1}>
                  <motion.div whileHover={{ y: -6 }} className="card p-6 flex flex-col items-center text-center group h-full">
                    {/* Avatar */}
                    <div className="relative mb-4">
                      <div className={`w-28 h-28 rounded-full bg-gradient-to-br ${AVATAR_COLORS[i % AVATAR_COLORS.length]} flex items-center justify-center text-3xl font-bold text-white shadow-primary overflow-hidden`}>
                        {doc.image
                          ? <Image src={getImageUrl(doc.image)} alt={doc.name} fill className="object-cover" />
                    ...
                    <p className="text-xs text-teal-600 font-semibold mb-1">{doc.qualifications}</p>
                    <p className="text-gray-500 text-xs mb-3">{doc.specialty}</p>

                    {/* Info pills */}
                    <div className="flex flex-wrap justify-center gap-1.5 mb-4">
                      <span className="badge bg-blue-50 text-blue-700">{doc.experience_years}+ yrs</span>
                      {doc.languages.slice(0, 2).map(lang => (
                        <span key={lang} className="badge bg-purple-50 text-purple-700 flex items-center gap-0.5">
                    ...
                    <div className="flex gap-1 mb-5">
                      {['M','T','W','T','F','S'].map((d, idx) => {
                        ...
                        return (
                          <span key={idx} className={`w-6 h-6 rounded-md text-xs font-bold flex items-center justify-center ${active ? 'bg-teal-100 text-teal-700' : 'bg-gray-100 text-gray-300'}`}>
                            {d}
                          </span>
                        )
                      })}
                    ...
                    <div className="mt-auto w-full flex flex-col gap-2">
                      <HotDocWidget mode="lightbox" buttonText="Book Appointment" buttonStyle="teal" className="w-full justify-center text-sm py-2.5 px-4" />
